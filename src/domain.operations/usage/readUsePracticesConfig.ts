...
import { readDeclarePracticesConfig } from '@src/domain.operations/declaration/readDeclarePracticesConfig';
    ...
    const declaredPractices = await (async () => {
      ...
      if (configInput.declarations.startsWith('npm:')) {
        ...
        const directoryExists = await doesDirectoryExist({
          directory: getAbsolutePathFromRelativeToConfigPath(
            expectedDirectoryRelativePath,
          ),
        });
        if (!directoryExists)
          throw new UserInputError(
            ...
            {
              potentialSolution:
                "make sure you've ran npm install in this project already. e.g., 'npm install'",
            },
          );
        ...
        const declarationsConfigFileExists = await doesFileExist({
          filePath: getAbsolutePathFromRelativeToConfigPath(
            expectedDeclarationsConfigRelativeFilePath,
          ),
        });
        ...
        return readDeclarePracticesConfig({
          configPath: getAbsolutePathFromRelativeToConfigPath(
            expectedDeclarationsConfigRelativeFilePath,
          ),
        });
      }
      ...
      const specifiedPath = getAbsolutePathFromRelativeToConfigPath(
        configInput.declarations,
      );
      ...
      if (!pathStat.isFile())
        throw new UserInputError(
          `path to declarations does not reference a real file. '${path}' does not exist`,
        );
