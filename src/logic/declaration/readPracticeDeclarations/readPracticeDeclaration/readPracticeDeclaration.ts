...
export const readPracticeDeclaration = async ({
  declaredPracticeDirectory,
}: {
  declaredPracticeDirectory: string;
}) => {
  // grab the file paths in this dir
  const paths = await listPathsInDirectory({
    directory: declaredPracticeDirectory,
  });

  // grab the name of the practice
  const practiceName = declaredPracticeDirectory.split('/').slice(-1)[0];
  if (!practiceName)
    throw new UnexpectedCodePathError('could not extract practice name', {
      declaredPracticeDirectory,
    });

  // find the "best-practice" dir, if one was defined
  const bestPracticeProjectCheck: ProjectCheckDeclaration | null =
    paths.includes('best-practice')
  ...
  const badPracticeProjectChecks: ProjectCheckDeclaration[] =
    await (async () => {
      if (!paths.includes('bad-practices')) return [];
      const badPracticesPaths = await listPathsInDirectory({
        directory: `${declaredPracticeDirectory}/bad-practices`,
      });
      return Promise.all(
