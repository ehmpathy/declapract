...
export const getFileCheckDeclaration = async ({
  ...
  const strictEqualsCheck = withOptionalityCheck(
    (foundContents: string | null) =>
      expect(foundContents).not.toBeNull() && expect(foundContents).toEqual(declaredBestPracticeContents),
  );
  const containsCheck = withOptionalityCheck(
    async (foundContents: string | null) =>
      expect(foundContents).not.toBeNull() && expect(foundContents).toContain(declaredBestPracticeContents),
  );
