...
import { readPracticeDeclaration } from '../readDeclarations/readPracticeDeclaration/readPracticeDeclaration';
...
export const readPracticeDeclarations = async ({
  ...
  const practices = await Promise.all(
    practiceDirectories.map((directory) =>
      readPracticeDeclaration({ declaredPracticeDirectory: `${declaredPracticesDirectory}/${directory}` }),
    ),
  );
  return practices;
};
