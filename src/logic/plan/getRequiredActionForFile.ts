...
import { FilePracticeEvaluation, hasPassed, isFixablePractice, RequiredAction } from '../../domain';

/**
 * returns what the required action is for a file, based on all of the evaluations for that file
 */
export const getRequiredActionForFile = ({ evaluations }: { evaluations: FilePracticeEvaluation[] }) => {
  ...
  const allFailedEvaluationsAreAutomaticallyFixable = failedEvaluations.every(isFixablePractice);
  if (allFailedEvaluationsAreAutomaticallyFixable) return RequiredAction.FIX_AUTOMATIC;
